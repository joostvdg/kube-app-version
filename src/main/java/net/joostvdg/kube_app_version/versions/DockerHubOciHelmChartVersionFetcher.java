...
public class DockerHubOciHelmChartVersionFetcher implements VersionFetcher {
    ...
    private static final Logger logger = LoggerFactory.getLogger(DockerHubOciHelmChartVersionFetcher.class);
    private final HttpClient httpClient;
    private final Map<String, List<String>> versionCache = new ConcurrentHashMap<>();
    private final ObjectMapper objectMapper = new ObjectMapper();
    private static final Pattern DOCKERHUB_OCI_PATTERN = Pattern.compile("^oci://([^/]+\\.docker\\.io)/(.+)$");
    ...
    public List<String> getAvailableVersions(AppArtifact artifact) throws Exception {
        if (!supports(artifact)) {
            logger.warn("DockerHubOciHelmChartVersionFetcher does not support artifact: {}", artifact.getSource());
        ...
        String authUrl = String.format("https://auth.docker.io/token?service=%s&scope=%s",
                AUTH_SERVICE, authScope);
        ...
        if (tokenResponse.statusCode() != 200) {
            logger.error("Failed to get Docker Hub auth token. HTTP status: {} - {}",
                    tokenResponse.statusCode(), tokenResponse.body());
            throw new RuntimeException("Failed to get Docker Hub auth token. HTTP status: " + tokenResponse.statusCode());
        }
        ...
        String tagsUrl = String.format("https://%s/v2/%s/tags/list",
                registryDomain, repositoryPath);
        ...
        if (tagsResponse.statusCode() != 200) {
            logger.error("Failed to get Docker Hub tags. HTTP status: {} - {}",
                    tagsResponse.statusCode(), tagsResponse.body());
            throw new RuntimeException("Failed to get Docker Hub tags. HTTP status: " + tagsResponse.statusCode());
        }
        ...
        List<Version> semVerList = versions.stream()
        ...
        logger.info("Found and sorted {} versions for Docker Hub OCI chart: {}", sortedVersionStrings.size(), source);
