...
public class GithubOciHelmChartVersionFetcher implements VersionFetcher {
    ...
    private static final Logger logger = LoggerFactory.getLogger(GithubOciHelmChartVersionFetcher.class);
    ...
    public List<String> getAvailableVersions(AppArtifact artifact) throws Exception {
        if (!supports(artifact)) {
            logger.warn("GithubOciHelmChartVersionFetcher does not support artifact: {}", artifact.getSource());
        ...
        String encodedPackagePath = URLEncoder.encode(packagePath, StandardCharsets.UTF_8).replace("+", "%20");

        String apiUrl = "https://api.github.com/users/" + username + "/packages/container/" + encodedPackagePath.replace("/", "%2F") + "/versions";
        ...
        if (response.statusCode() != 200) {
            logger.error("Failed to fetch {}. HTTP status: {} - {}", apiUrl, response.statusCode(), response.body());
            throw new RuntimeException("Failed to fetch " + apiUrl + ". HTTP status: " + response.statusCode());
        }
        ...
        logger.info("Found and sorted {} versions for GitHub OCI chart: {}", sortedVersionStrings.size(), source);
